...
import type { NodeData, ServiceCell } from './cells';
import type { ServiceStatus } from '@/theme';

/**
 * A generated service map: 1,200 nodes and ~1,600 links.
 *
 * Generated rather than committed as JSON — the same graph as a saved file
 * would be several megabytes — and seeded, so the diagram is identical on every
 * load and two runs of the FPS readout can be compared.
 *
 ...
 * 1,200 nodes in the viewport at once. Virtual rendering cannot help there —
 * everything really is on screen — so what a node costs to draw is the only
...
/** Nodes per row. 1,200 over 30 columns is 40 rows. */
const COLUMNS = 30;
...
/** The node at `(row, column)`, or `null` when that slot is off the map. */
 ...
 * merely cheap: roughly one node in eight is off, so zoomed out the map is a
...
function makeData(index: number, random: () => number): NodeData {
...
export function generateGraph(seed = 20260915): readonly ServiceCell[] {
    const random = createRandom(seed);
    const pick = (span: number) => Math.floor(random() * (span * 2 + 1)) - span;

    const cells: ServiceCell[] = [];
    ...
    const connect = (sourceIndex: number, targetIndex: number): void => {
        ...
        seen.add(key);
        cells.push(makeLink(`l${seen.size}`, `n${sourceIndex}`, `n${targetIndex}`));
    };

    // The spine: every node below the first row is reached from one above it,
    // so the map is connected and reads as a flow rather than as a mesh.
