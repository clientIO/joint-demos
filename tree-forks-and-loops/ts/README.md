...
A tree laid out by `layout.TreeLayout` in which a node can be a *group*: a container with a start node, some content and an end node. A *fork group* holds two branches that converge into the end node — a fork/join. A *loop group* holds a tree whose end node links back to the start node — a cycle, drawn as a dashed return link up the left side of the group. An *if group*, labelled *Condition*, holds one branch and a `no` line that skips it, running down the right side of the group outside its box — the return link of a loop, the other way round. The tree connects to a group as a single node, so subgraphs that a tree layout cannot handle on its own fit into the tree. Groups nest and collapse.
...
- **Conditional branches** -- an `if` group holds a branch like any other group, standing on the axis and growing straight down, and a second way out of its start node: the `no` line, which skips the branch and joins the flow again at the end node. It is routed by hand out of the start node to the right, down the right side of the group outside of its box and into the end node from the right -- the return link of a loop mirrored, running with the flow instead of against it. Nothing hangs beside the group, so the axis of the `if` stays under its parent and the room it asks for is one gap on each side, the same as a loop. The first node of the branch is given an `offset`, which the tree layout adds to the gap from its parent, so that the `yes` label and the toggle of the group have the stretch below the start node to themselves.
...
| `src/shapes.ts` | The `Node`, `Group` and `Link` cell types and the layout metrics. A node with a `role` is a gate of a group: the start node is labelled with the kind of its group, the end node is a point of no size. A group has a `kind`, fork, loop or if; it is a translucent slab, expanded around its content or shrunk to a node labelled with its kind. The return link of a loop is a dashed `Link`, the two ways out of an `if` are `Link`s labelled `yes` and `no`, and a `Link` that joins a branch back into the flow is marked as such |
| `src/layout.ts` | The bottom-up layout: one `TreeLayout` pass per expanded group, then one for the outer tree; the routes of the two links that run outside a box, the return link of a loop and the `no` line of an `if`, and the room kept for them. Also the visibility predicates, one for the layout and one for the paper |
...
1. A group embeds its start node, its content, its end node and the inner links: for a fork group two branch nodes and four links (`start → a`, `start → b`, `a → end`, `b → end`); for a loop group one node, `start → a`, `a → end` and the return link `end → start`; for an `if` group one node, `start → a`, `a → end` and the `no` line `start → end`. The outer links (`parent → group`, `group → child`) are attached to the group element.
