...
import type { SelectionApi } from '@joint/react-plus';
...
type ViewControllerArgs = {
    ...
    startSelectionRegion: SelectionApi['startSelectionRegion'];
};
...
function onPaperBlankPointerDown(context: ViewControllerArgs, evt: dia.Event) {
    const { paper, paperScroller, keyboard, selection, startSelectionRegion } = context;
...
function onElementPointerDown(context: ViewControllerArgs, elementView: dia.ElementView, evt: dia.Event) {
    const { keyboard, startSelectionRegion } = context;
    ...
    const originSwimlaneId = elementView.model.id;

    // Select only elements inside the swimlane the shift selection started on,
    ...
    startSelectionRegion({
        filter: ({ model }) => {
            ...
            return parent.id === originSwimlaneId;
        }
    });
}
