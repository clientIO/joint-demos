...
export class AppLinkView extends dia.LinkView<AppLink> {
    ...
    changeStyle(linkType: LinkType): void {
        const { router, attrs } = linkTypeStyles[linkType];
        const resetAttrs = {
            line: {
                sourceMarker: null,
                targetMarker: null,
    ...
    protected dragArrowheadEnd(evt: dia.Event, x: number, y: number): void {
        ...
        if (!canLinkExist || isEndSame) {
            ...
            this.model.attr('line/stroke', this.savedLinkAttributes.attrs.line?.stroke, { skipHistory: true });
            this.model.vertices(this.savedLinkVertices, { skipHistory: true });
        }
    ...
    private applyLinkProperties(attrs: dia.Link.Attributes, router: string, vertices: dia.Link.Vertex[]): void {
        this.model.attr(attrs, { skipHistory: true });
        this.model.router(router, { useVertices: true }, { skipHistory: true });
        this.model.vertices(vertices, { skipHistory: true });
    }
}
