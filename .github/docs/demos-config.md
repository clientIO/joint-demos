...
| `skipScreenshotComparison` | `boolean` | compare-screenshots script | Excludes the demo from screenshot regression comparison only. Unlike `skip`, it has no effect on build-demos.sh, test-demos.sh or screenshot-demos.mjs — the demo still builds, tests and deploys normally. |
...
- [`.github/scripts/test-demos.sh`](../scripts/test-demos.sh) — the test script, which reads the same `variant`, `skip` and `buildFlags` fields
- [`.github/docs/test-demos.md`](./test-demos.md) — documentation for the test script
