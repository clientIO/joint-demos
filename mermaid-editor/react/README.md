...
- **Delete from the canvas** — Delete or Backspace on a selected node or edge, or the trash button in its toolbar, rewrites the source rather than the cells: a node is cut out of every chain it sits in (`a --> b --> c` minus `b` leaves `a` and `c` declared, shapes and labels intact), a fan keeps its other members, `style` / `class` / `click` lines lose the node, and `linkStyle` indices are renumbered for the edges that went. Each removal is one undo step in the editor. `src/mermaid/remove.ts` does the rewrite and is covered by Node tests
