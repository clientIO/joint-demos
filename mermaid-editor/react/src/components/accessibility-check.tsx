/**
 * "Check accessibility" pill — the same affordance as the Data Modeling, AI
 * Workflow Builder and BPMN demos: a link that runs the current URL through
 * accessibilitychecker.org. Floats bottom-left, diagonally opposite the zoom
 * cluster, so the two never collide.
 */
...
export function AccessibilityCheck() {
    return (
        ...
        >
            <span className="accessibility-check-ping" aria-hidden>
                <span className="accessibility-check-ping-wave" />
                <span className="accessibility-check-ping-dot" />
            </span>
            <svg viewBox="0 0 512 512" className="accessibility-check-icon" fill="currentColor" aria-hidden>
