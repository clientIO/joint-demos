...
export function createTreeLayout(graph: dia.Graph, options: Partial<layout.TreeLayout.Options> = {}): layout.TreeLayout {
    ...
    return new layout.TreeLayout({
        ...
        updateVertices: (link, vertices, opt) => {
            ...
            // The bar is halfway between the parent and the child. The layout
            // leaves no vertices when the child is right below the parent.
            const targetBBox = target.getBBox();
            const barY = vertices.length > 0
                ? vertices[0].y
                : (direction === 'T' ? targetBBox.corner().y + PARENT_GAP / 2 : targetBBox.y - PARENT_GAP / 2);
 ...
 * into, placed below the tree: a horizontal bar halfway above the gate, like
 * the one the tree layout draws below a parent, mirrored. A leaf right above
...
export function joinLeavesInto(graph: dia.Graph, gate: Node): void {
    ...
    const barY = gateBBox.y - PARENT_GAP / 2;
 ...
 * tree has been moved: a horizontal bar halfway below the gate, the one the
 * tree layout draws below a parent. A root right below the gate connects
 * straight. Links to the other gate are left alone.
 */
export function forkChildrenFrom(graph: dia.Graph, gate: Node): void {
    const gateBBox = gate.getBBox();
    const gateX = gateBBox.center().x;
    const barY = gateBBox.corner().y + PARENT_GAP / 2;
