...
/** The command a step runs - one line or several - as code on a chip: the programs in bold, the flags tinted (see `command.ts`). */
function RunLine({ run }: { run: string }): ReactNode {
    return (
        <code className="run">
            {tokenizeCommand(run).map((token, index) => (token.kind === 'plain' ? token.text : <span key={index} className={token.kind}>{token.text}</span>))}
        </code>
    );
}
