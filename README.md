...
Every demo it touches also gets an `overrides` block listing all the local packages, so a package reached only transitively is caught too. `@joint/core` is the case that matters: `@joint/plus` depends on it by range and almost no demo declares it, so without the override npm resolves it from the registry and the run quietly tests a *released* core against a local `@joint/plus`. The version alone will not tell you which copy you got — a local build and the published package often share one — so check where it was resolved from:
